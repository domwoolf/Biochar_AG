...
# Script to generate publication-quality display items for the BiocharAG manuscript.
...
load_region_data <- function(region_name) {
    ...
    if (!dir.exists(gis_path)) {
        gis_path <- "GIS/processed/"
        if (!dir.exists(gis_path)) stop("Could not locate GIS/processed/ directory.")
    }

    prefix_map <- list(
        "US" = list(base = "demo", dist = "us"),
    ...
    if (region_name == "US" && file.exists(file.path(gis_path, "us_elec_price.tif"))) {
    ...
    if (region_name == "US" && file.exists(file.path(gis_path, "soil_ph.tif"))) {
        ph <- terra::rast(file.path(gis_path, "soil_ph.tif"))
    }
    cec <- terra::rast(file.path(gis_path, paste0(p_base, "_soil_cec.tif")))
    if (region_name == "US" && file.exists(file.path(gis_path, "soil_cec.tif"))) {
    ...
    admin0 <- if (file.exists(a0_path)) sf::st_read(a0_path, quiet = TRUE) else NULL
    admin1 <- if (file.exists(a1_path)) sf::st_read(a1_path, quiet = TRUE) else NULL
    ...
    return(list(template = bm, layers = layers, admin0 = admin0, admin1 = admin1))
}

run_scenario <- function(template, layers, params) {
    # Run the base TEA (which takes ~10-20 seconds per tech)
    bes <- BiocharAG::run_spatial_tea(template, params, layers, fun = BiocharAG::calculate_bes, collection_radius_km = 50)
    beccs <- BiocharAG::run_spatial_tea(template, params, layers, fun = BiocharAG::calculate_beccs, collection_radius_km = 50)
    bebcs <- BiocharAG::run_spatial_tea(template, params, layers, fun = BiocharAG::calculate_bebcs, collection_radius_km = 50)

    net_stack <- c(bes[["Net_Value_USD"]], beccs[["Net_Value_USD"]], bebcs[["Net_Value_USD"]])
    names(net_stack) <- c("BES", "BECCS", "BEBCS")

    abate_stack <- c(bes[["Abatement_tCO2"]], beccs[["Abatement_tCO2"]], bebcs[["Abatement_tCO2"]])
    names(abate_stack) <- c("BES", "BECCS", "BEBCS")

    opt_idx <- terra::app(net_stack, which.max)

    return(list(net = net_stack, abate = abate_stack, opt = opt_idx))
}
...
get_linear_baseline <- function(template, layers, base_params) {
    ...
    return(res0) # Returns net at C=0, and abatement
}

# --- FIGURE GENERATORS ---

# Figure 1: Scale vs. Sink Bivariate Map
generate_fig1_physical_boundary <- function(dat, region_name, save_map = FALSE) {
    ...
    stack_df <- terra::as.data.frame(c(dat$layers$biomass_density, dat$layers$dist_onshore, res$opt), xy = TRUE, na.rm = TRUE)
    ...
    p <- ggplot(stack_df, aes(x = dist, y = biomass)) +
        geom_point(aes(color = tech), alpha = 0.5, size = 1) +
        scale_color_manual(values = c("BES" = "#1f77b4", "BECCS" = "#d62728", "BEBCS" = "#2ca02c")) +
    ...
    if (any(stack_df$tech == "BECCS", na.rm = TRUE)) {
        p <- p + geom_density_2d(data = stack_df[!is.na(stack_df$tech) & stack_df$tech == "BECCS", ], color = "black", alpha = 0.7)
    }

    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig1_Physical_Boundary.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
    } else {
        print(p)
    }
    return(p)
...
generate_fig2_booster_penalty <- function(dat, region_name, save_map = FALSE) {
    ...
    stack_df <- terra::as.data.frame(c(dat$layers$biomass_density, dat$layers$dist_onshore, res$opt, cell_area), na.rm = TRUE)
    ...
    beccs_df <- stack_df %>%
        filter(tech == "BECCS") %>%
        arrange(dist) %>%
        mutate(cumulative_biomass = cumsum(cell_biomass))
    ...
    beccs_df$percent_national <- (beccs_df$cumulative_biomass / total_biomass) * 100

    p <- ggplot(beccs_df, aes(x = dist, y = percent_national)) +
        geom_line(color = "#d62728", linewidth = 1.5) +
        geom_vline(xintercept = 700, linetype = "dashed", color = "black") +
        annotate("text", x = 750, y = max(beccs_df$percent_national, na.rm = TRUE) * 0.5, label = "700km Booster Threshold", angle = 90) +
        theme_minimal(base_size = 14) +
        labs(
            title = paste0("BECCS Addressable Biomass vs Distance to Sink - ", region_name),
            x = "Distance to Sink (km)",
            y = "% of Total Available Biomass"
        )

    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig2_Booster_Penalty_CDF.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
    } else {
        print(p)
    }
    return(p)
...
generate_fig3_evaporation <- function(dat, region_name, save_map = FALSE) {
    ...
    all_df$dr_label <- factor(all_df$dr_label, levels = c("Discount Rate: 2%", "Discount Rate: 8%", "Discount Rate: 15%"))

    plt <- ggplot() +
        geom_tile(data = all_df, aes(x = x, y = y, fill = tech))
    ...
    plt <- plt +
        ...
        labs(fill = "Optimal Technology", title = paste0("Financial Gravity: Evaporation of BECCS - ", region_name))

    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig3_Evaporation_Maps.png"), plt, width = 10, height = 7, bg = "white", dpi = 300)
    } else {
        print(plt)
    }
    return(plt)
...
generate_fig4_capital_wedge <- function(dat, region_name, save_map = FALSE) {
    ...
    for (dr in dr_seq) {
        ...
        stack_df <- terra::as.data.frame(c(dat$layers$biomass_density, res$opt, cell_area), na.rm = TRUE)
        ...
        agg <- stack_df %>%
            group_by(tech) %>%
            summarize(total_biomass = sum(cell_biomass, na.rm = TRUE), .groups = "drop")
        agg$dr <- dr * 100
    ...
    p <- ggplot(df_plot, aes(x = dr, y = total_biomass / 1e6, fill = tech)) +
        geom_area(alpha = 0.8) +
        scale_fill_manual(values = c("BES" = "#1f77b4", "BECCS" = "#d62728", "BEBCS" = "#2ca02c")) +
        theme_minimal(base_size = 14) +
        labs(
    ...
    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig4_Capital_Wedge.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
    } else {
        print(p)
    }
    return(p)
...
generate_fig5_cprice_threshold <- function(dat, region_name, save_map = FALSE) {
    message("Generating Figure 5: Carbon Price Threshold Map for ", region_name, "...")
    ...
    p_bebcs[den_bebcs <= 0] <- Inf # If abatement isn't higher, it will never win
    p_bebcs[p_bebcs < 0] <- Inf # If it's negative, it already wins at $0 (unlikely for CDR vs BES)
    ...
    p <- ggplot() +
        geom_tile(data = df_map, aes(x = x, y = y, fill = threshold))
    if (!is.null(dat$admin0)) {
        p <- p + geom_sf(data = dat$admin0, fill = NA, color = "black", linewidth = 0.5)
    }
    if (!is.null(dat$admin1)) {
        p <- p + geom_sf(data = dat$admin1, fill = NA, color = "black", linetype = "dotted", linewidth = 0.2)
    }
    p <- p +
        coord_sf(crs = 4326) +
        scale_fill_viridis_c(option = "magma", direction = -1, limits = c(0, 300), oob = scales::squish) +
        theme_void(base_size = 14) +
    ...
    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig5_Threshold_Map.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
    } else {
        print(p)
    }
    return(p)
...
generate_fig6_macc <- function(dat, region_name, save_map = FALSE) {
    ...
    stack_df <- terra::as.data.frame(c(dat$layers$biomass_density, cell_area, npv0, abate), xy = TRUE, na.rm = TRUE)
    # Names: biomass, area, BES_n, BECCS_n, BEBCS_n, BES_a, BECCS_a, BEBCS_a
    names(stack_df)[3:10] <- c("biomass", "area", "NPV0_BES", "NPV0_BECCS", "NPV0_BEBCS", "A_BES", "A_BECCS", "A_BEBCS")
    ...
    current_tech <- rep("BES", nrow(stack_df))
    # It is technically possible BEBCS or BECCS wins at 0, let's verify:
    val0_bes <- stack_df$NPV0_BES
    val0_beccs <- stack_df$NPV0_BECCS
    val0_bebcs <- stack_df$NPV0_BEBCS

    for (i in 1:nrow(stack_df)) {
        v <- c(val0_bes[i], val0_beccs[i], val0_bebcs[i])
        best <- which.max(v)
        current_tech[i] <- c("BES", "BECCS", "BEBCS")[best]
    }
    ...
    if (length(macc_points) > 0) {
        ...
        agg_macc <- macc_df %>%
            group_by(price) %>%
            summarize(marginal_abatement = sum(abatement, na.rm = TRUE), .groups = "drop") %>%
            arrange(price) %>%
            mutate(cumulative_abatement = cumsum(marginal_abatement))

        p <- ggplot(agg_macc, aes(x = cumulative_abatement / 1e6, y = price)) +
        ...
        if (save_map) {
            ggsave(paste0("figures/", region_name, "_Fig6_MACC.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
        } else {
            print(p)
        }
        return(p)
...
generate_fig7_agronomic_bridge <- function(dat, region_name, save_map = FALSE) {
    ...
    stack_df <- terra::as.data.frame(c(res_no$opt, res_ag$opt), xy = TRUE, na.rm = TRUE)
    ...
    stack_df$status[stack_df$tech_no != stack_df$tech_ag] <- paste0("Switched to ", stack_df$tech_ag[stack_df$tech_no != stack_df$tech_ag])
    ...
    p <- ggplot() +
        geom_tile(data = stack_df, aes(x = x, y = y, fill = status))
    if (!is.null(dat$admin0)) {
        p <- p + geom_sf(data = dat$admin0, fill = NA, color = "black", linewidth = 0.5)
    }
    if (!is.null(dat$admin1)) {
    ...
    p <- p +
        ...
        labs(
            ...
            subtitle = "Difference in optimal tech with vs without Mechanistic Biochar Ag Value",
            fill = "Impact"
        )

    if (save_map) {
        ggsave(paste0("figures/", region_name, "_Fig7_Agronomic_Bridge.png"), p, width = 8, height = 6, bg = "white", dpi = 300)
    } else {
        print(p)
    }
    return(p)
...
if (sys.nframe() == 0) {
    ...
    for (r in regions) {
        ...
        generate_fig1_physical_boundary(dat, r, save_map)
