...
The BECCS pathway augments the standard combustion baseline with a post-combustion carbon capture unit. The addition of this unit imposes a significant parasitic energy penalty, primarily driven by solvent regeneration and $\text{CO}_2$ compression requirements. Consequently, the net electrical conversion efficiency is modelled as the BES baseline minus a fixed penalty (`beccs_eff_penalty` = 8 percentage points), i.e. a drop from 30% to 22%, consistent with the 4--11 percentage point range reported by Woolf et al. (2016).[^5]

[^5]: **Carbon Capture Efficiency:** BECCS efficiency is derived as `bes_energy_efficiency - beccs_eff_penalty` rather than sampled independently, so Monte Carlo draws keep BECCS below BES. The high-ash fuel multiplier is applied afterwards, scaling both technologies proportionally.
...
-   **BECCS:** Abatement includes the physical $\text{CO}_2$ captured and injected into the geological sink (assuming a 90% capture rate) plus the grid displacement credit. However, because the parasitic energy load of the capture process reduces the net electrical efficiency (from 30% to 22%), the total energy exported—and thus the resulting grid displacement credit—is proportionally lower for BECCS than for pure BES\[\^5\].
-   **BEBCS:** Abatement combines the grid displacement credit with the permanent $\text{CO}_2$ equivalent of the biochar carbon retained in the soil ($F_{perm}$).
...
| `beccs_eff_penalty` | BECCS efficiency penalty relative to BES | Scalar | 0.08 | Fraction | Woolf et al. (2016) |
...
| **Biochar & Agronomic** |  |  |  |  |  |
| `h_c_org` | Molar H:C ratio for permanence (derived from `py_temp` via pyrolysis char composition; 0.35 at 500 °C) | Derived | 0.35 | Molar Ratio | Woolf et al. (2021) |
